...
class CampaignService
{
    ...
    private function storeContentVariations(Campaign $campaign, array $contentVariations): void
    {
        ...
        $this->setCachedData($cacheKey, json_encode($variations), 3600); // 1 hour TTL
    }
    ...
    private function storeRecipientData(Campaign $campaign, array $recipientData): void
    {
        ...
        $this->setCachedData($cacheKey, json_encode($recipientData), 3600); // 1 hour TTL
    ...
    private function storeSenderList(Campaign $campaign, $senders): void
    {
        ...
        $this->setCachedData($cacheKey, json_encode($senderList), 3600); // 1 hour TTL
    }
    ...
    public function startCampaign(Campaign $campaign): array
    {
        ...
        try {
            ...
            $this->logError('Failed to start campaign', [
                'campaign_id' => $campaign->id,
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Send single email directly without queue for immediate dispatch
     */
    public function sendSingleEmailDirect(array $data): array
    {
        ...
        try {
            ...
            $this->logError('Single email sending failed', [
                'recipient' => $data['to'],
                'error' => $e->getMessage(),
            ]);
    ...
    public function resumeCampaign(Campaign $campaign): array
    {
        ...
        try {
            ...
            if ($queueWorkerAvailable) {
                ...
                $this->logInfo('Campaign resumed and processed synchronously', [
                    'campaign_id' => $campaign->id,
        ...
        try {
            ...
            foreach ($recipientsToProcess as $index => $recipient) {
                try {
                    ...
                    // Don't increment emailsSent here - it should only be incremented when emails are actually sent
            ...
            if (!empty($jobs)) {
                ...
                // For queued jobs, SendEmailJob will handle updating campaign stats individually
                $emailsSent = 0; // Reset since we don't know actual sent count yet
                $emailsFailed = 0;
            }

            // No campaign statistics update needed here - SendEmailJob handles all counting
            ...
            // Get updated campaign data for comprehensive logging
